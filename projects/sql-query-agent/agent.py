...
Connects to SQLite and answers natural-language questions by generating
...
def sqlite_uri(db_path: str, read_only: bool = True) -> str:
    ...
    return f"sqlite:///file:{quote(abs_path)}?mode=ro&uri=true" if read_only else f"sqlite:///{abs_path}"

def build_agent(db_path: str, read_only: bool = True):
    db = SQLDatabase.from_uri(sqlite_uri(db_path, read_only))
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
    toolkit = SQLDatabaseToolkit(db=db, llm=llm)
    return create_sql_agent(llm=llm, toolkit=toolkit, agent_type=AgentType.ZERO_SHOT_REACT_DESCRIPTION, verbose=False), db

def main():
    parser = argparse.ArgumentParser(description="SQL Query Agent")
    parser.add_argument("--db", default="demo.sqlite")
    parser.add_argument("--question")
    parser.add_argument("--allow-write", action="store_true")
    args = parser.parse_args()
    if args.db == "demo.sqlite" and not os.path.exists(args.db):
        create_demo_database(args.db)
    agent, db = build_agent(args.db, read_only=not args.allow_write)
    print(f"Connected to {args.db}; tables: {', '.join(db.get_table_names())}")
    if args.question:
